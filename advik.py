...
def speak(audio):
	...
	speak_assis.setProperty('voice', voices[1].id)
...
def windows_7():
	...
	sbp.call(["C:/Users/ksudh/Documents/Virtual Machines/Windows 7/Windows 7.vmx"], shell=True)

def payload():
	sbp.call('pyinstaller ' + '--noconfirm ' + '--onefile ' + '--windowed ' + '--icon '+ '"exeico.ico" ' + '"D:/Python/MYSP/services.py"')
...
def created():
	speak('I was created by Sudhanshu')
...
def browse_net():
	...
	try:
		...
		if 'google' in gg:
			speak('Opening '+bs)
			webbrowser.get("C:/Program Files/Google/Chrome/Application/chrome.exe %s").open('https://www.google.com/search?q='+bs)
			pass
		elif 'site' in gg:
			speak('Opening '+bs)
			webbrowser.get("C:/Program Files/Google/Chrome/Application/chrome.exe %s").open(bs)
			pass
	except Exception:
		pass
...
def can_do():
	...
	speak('I can Join Class, Just say join class')
	speak('Generation of Payload!, Just say generate payload')
	...
	speak('Dark Web, Just say I want to surf dark web')
	speak('Read your last book, just say Please read me the last book')
	speak('I can start windows 7, just say start windows 7')
